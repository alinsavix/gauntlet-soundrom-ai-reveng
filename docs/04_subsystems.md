...
RESET $5A25
  → initialization $4002
  → RAM/ROM tests, queues, sound chips, channel pools
  → main loop $40C8

Main CPU command → NMI $57B0
  → direct status query (commands 3, 6, 7), or
  → incoming command ring
  → command dispatcher $432E
     → type 7 shared sequence path
     → type 11 TMS5220 speech path
     → control/stop/fade/output handlers

Video-derived IRQ $4187
  → acknowledge $1830
  → heartbeat/timers
  → audio update $41C8
  → alternating POKEY/YM2151 hardware update
  → repeated speech-status/FIFO service
  → board/coin control $8381
```
